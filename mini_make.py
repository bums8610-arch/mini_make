...
def fetch_url_text(url: str, timeout: int = 25) -> str:
    """외부 의존성 없이(=requests 없이) HTML을 가져오는 최소 함수."""
    req = Request(url, headers={"User-Agent": UA, "Accept-Language": "ko-KR,ko;q=0.9"})
    try:
        with urlopen(req, timeout=timeout) as r:
            raw = r.read()
        # 대체로 UTF-8, 실패하면 깨진 글자는 무시
        return raw.decode("utf-8", errors="ignore")
    except (HTTPError, URLError) as e:
        log(f"[기사] fetch 실패: {type(e).__name__} {e}")
        return ""


def _meta_content(html: str, prop_or_name: str) -> str:
    """
    <meta property="og:title" content="..."> 또는 <meta name="description" content="...">
    를 대충이라도 뽑는(취약하지만 가벼운) 정규식.
    """
    if not html:
        return ""
    # property / name 둘 다 커버
    pattern = re.compile(
...
def fetch_article_brief(article_url: str) -> dict[str, str]:
    """
    (원래 코드에서 호출하지만 정의가 없어서 NameError가 났던 함수)
    기사 페이지에서 OG 메타 정도만 뽑아, OpenAI 대본 품질을 조금이라도 올림.
    실패해도 빈 dict 반환(파이프라인 계속 진행).
    """
...
def build_60s_shorts_script_openai(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    strict=True 스키마 요건(추가필드 금지)을 만족하도록 additionalProperties를 명시.
    """
    ...
    system = (
        ...
        "사용자가 준 정보(제목/메타설명/링크) 밖의 사실을 단정하지 말고, "
        "정보가 부족하면 '제목/요약 기준'이라고 표현해라. "
        "과장/루머/비방 금지. "
        "정확히 6구간(0-2s, 2-10s, 10-25s, 25-40s, 40-55s, 55-60s). "
        "voice는 말로 읽기 좋게 짧게, onscreen은 12자 이내."
    )
...
def node_load_inputs(ctx: dict[str, Any]):
    ...
    # 기사 메타는 실패해도 진행(취약성 감소)
    brief: dict[str, str] = {}
    try:
        brief = fetch_article_brief(article_url)
    except Exception as e:
        log(f"[기사] brief 파싱 실패(무시하고 진행): {e}")
